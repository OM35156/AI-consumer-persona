...
from digital_twin.persona.builder import PersonaBuilder
...
class AppState:
    ...
    def __init__(self) -> None:
        self.personas: list[PhysicianPersona] = []
        self.scenarios: list[PromotionScenario] = []
        self.surveys: list[SurveyInstrument] = []
        self.holdout_physicians: list[Physician] = []
        self.simulator: Simulator = Simulator()
        self._persona_map: dict[str, PhysicianPersona] = {}
    ...
    def initialize(self, config_profile: str = "poc") -> None:
        ...
        # データ読込
        physicians, surveys, scenarios = load_dataset(
            config.data.get("raw_dir", "./data/dummy")
        )
        logger.info("Loaded %d physicians, %d surveys, %d scenarios",
                     len(physicians), len(surveys), len(scenarios))

        # 匿名化
        anon_config = config.get("anonymization", {})
        physicians, _ = anonymize_dataset(
            physicians,
            k=anon_config.get("k_anonymity", 5),
            suppress_threshold=anon_config.get("suppress_threshold", 3),
        )

        # ホールドアウト分割
        holdout_ratio = config.get("evaluation", {}).get("holdout_ratio", 0.3)
        train, holdout = split_holdout(physicians, holdout_ratio=holdout_ratio)
        self.holdout_physicians = holdout
    ...
    def get_persona(self, persona_id: str) -> PhysicianPersona | None:
